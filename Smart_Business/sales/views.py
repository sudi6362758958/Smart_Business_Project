...
from .models import Business, Invoice, InvoiceItem  # adjust names if different
...
def invoice_list(request):
    ...
    # Base queryset
    qs = Invoice.objects.filter(business=business).order_by('-date')

    # Prefetch invoice items and their products to avoid N+1 queries.
    # If your related_name is not 'items' or your item model is named differently, change accordingly.
    try:
        qs = qs.prefetch_related(Prefetch('items', queryset=InvoiceItem.objects.select_related('product')))
    except Exception:
        # if prefetch fails (different related name), ignore and continue — not fatal
        pass

    # --- Filters from GET ---
    q = request.GET.get('q', '').strip()           # combined search: customer name OR invoice number OR product name
    status = request.GET.get('status', '').strip()
    date_from = request.GET.get('date_from', '').strip()
    date_to = request.GET.get('date_to', '').strip()

    if q:
        # Try several filter combos in a safe order:
        # 1) invoice_no, related customer.name (if FK), product name
        # 2) invoice_no, customer text field, product name
        # 3) invoice_no and product name only (fallback)
        tried = False
        try:
            qs = qs.filter(
            ...
            # customer__name may not exist (customer might be text field), try next
            pass
        ...
        if not tried:
            # last resort: search invoice_no and product name
            qs = qs.filter(
                Q(invoice_no__icontains=q)
                | Q(items__product__name__icontains=q)
            ).distinct()
    ...
    try:
        ...
        per_page = 25
    per_page = per_page if per_page in (5, 25, 50, 100) else 25
...
from .models import Invoice  # and InvoiceItem if you use it explicitly
from django.db.models import Prefetch

@login_required
def invoice_export(request):
    """
    Export selected invoices (ids via GET ?ids=1,2,3) as CSV.
    Only exports invoices belonging to the current user's business.
    """
    ...
    if not id_list:
        return HttpResponse("No valid invoice ids provided.", status=400)

    # Fetch invoices — ensure they belong to current user's business/owner
    # Adjust lookup if Invoice has business foreign key different name
    qs = Invoice.objects.filter(pk__in=id_list, business__owner=request.user).prefetch_related('items__product')

    if not qs.exists():
        return HttpResponse("No invoices found for given ids.", status=404)
    ...
    # Header rows
    writer.writerow(['Invoice No', 'Date', 'Customer', 'Total', 'Paid', 'Status', 'Remaining', 'Product', 'Quantity'])

    # For each invoice, write one row per line item (if you prefer one-row-per-invoice, change below)
    for inv in qs:
        items = getattr(inv, 'items', None)
        if items is None:
            # If there is no related 'items' manager, just write invoice row
            writer.writerow([inv.invoice_no, inv.date, str(inv.customer), inv.total, inv.amount_paid, inv.status, inv.remaining_amount, '', ''])
            continue
        ...
        if not inv_items:
            ...
            for it in inv_items:
                product_name = getattr(it.product, 'name', str(it.product)) if getattr(it, 'product', None) else ''
                qty = getattr(it, 'quantity', '')
                writer.writerow([inv.invoice_no, inv.date, str(inv.customer), inv.total, inv.amount_paid, inv.status, inv.remaining_amount, product_name, qty])
...
def invoice_pdf(request, pk):
    ...
    pdf_file = render_invoice_pdf(invoice)
...
def invoice_email(request, pk):
    ...
    if invoice.status == "paid":
        subject = f"Invoice {invoice.invoice_no} – Payment Received"
        body = (
            f"Dear {invoice.customer.name},\n\n"
            f"Thank you for your payment.\n"
            f"Invoice {invoice.invoice_no} has been fully paid.\n\n"
        ...
        subject = f"Invoice {invoice.invoice_no} – Partial Payment"
        body = (
            f"Dear {invoice.customer.name},\n\n"
            f"We have received partial payment for Invoice {invoice.invoice_no}.\n\n"
            f"Total: ₹{invoice.total}\n"
            f"Paid: ₹{invoice.amount_paid}\n"
            f"Balance: ₹{invoice.remaining_amount}\n\n"
            f"Please clear the remaining amount.\n\n"
            f"Regards,\n{invoice.business.name}"
        )

    else:  # pending
        subject = f"Invoice {invoice.invoice_no} – Payment Pending"
        body = (
            f"Dear {invoice.customer.name},\n\n"
            f"Payment for Invoice {invoice.invoice_no} is pending.\n\n"
    ...
    messages.success(
        request,
        f"Invoice emailed successfully to {invoice.customer.email}"
    )
    return redirect("sales:invoice_list")
