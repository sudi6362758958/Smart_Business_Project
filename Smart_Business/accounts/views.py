...
def register_business(request, pk=None):
    ...
    - Anyone (including anonymous) can submit a new business.
      * If anonymous and provided owner_email+password, a new inactive User will be created and assigned.
      * If anonymous and did NOT provide owner info, the business will have owner=None and wait for admin assignment.
    - Logged-in users become the owner of the created business (owner set to request.user).
    - Editing an owned business is allowed only to the owner or staff.
    """
    edit = bool(pk)
    business = None
    ...
    if edit:
        business = get_object_or_404(Business, pk=pk)
        # Only allow owner or staff to edit an owned business
        if business.owner:
            ...
            # business has no owner yet; allow staff to edit, allow anonymous only if you want (here we require auth)
            if not request.user.is_authenticated and not request.user.is_staff:
                return HttpResponseForbidden("Login required to edit this unowned business.")

    # instantiate form and pass request.user so form can enforce owner field requirements
    if request.method == 'POST':
        form = BusinessRegistrationForm(request.POST, request.FILES, user=request.user, instance=(business if edit else None))

        if form.is_valid():
            new_business = form.save(commit=False)

            # 1) If the submitter is authenticated: assign them as owner (unless staff will assign via admin later)
            if request.user.is_authenticated:
                # Assign the logged-in user as owner (safe because authenticated)
                new_business.owner = request.user

            else:
                # 2) Anonymous submitter: attempt to create a User if owner_email + password are provided
                owner_email = form.cleaned_data.get('owner_email')
                owner_name = form.cleaned_data.get('owner_name')
                pwd = form.cleaned_data.get('password')

                if owner_email and pwd:
                    # create a new user but keep them inactive until admin approves
                    try:
                        with transaction.atomic():
                            ...
                            # keep them inactive so admin approval is required
                        ...
                        # unlikely because form validated uniqueness, but handle gracefully
                        messages.error(request, "Could not create owner account — please try again or contact admin.")
                        return render(request, 'accounts/register_business.html', {'form': form, 'edit': edit, 'business': business})

                else:
                    # anonymous submit without owner fields: leave owner as None
                    new_business.owner = getattr(business, 'owner', None)

            # keep business unapproved until admin approves (you may change this as needed)
            ...
            try:
                ...
                # ignore m2m errors for now
                pass

            messages.success(request, "Business saved successfully. The admin will review and approve it shortly.")

            # optionally: if we created a user in the anonymous flow, notify admin or the user by email here

            try:
                return redirect(reverse('accounts:owner_dashboard', args=[new_business.id]))
        ...
        # GET
        form = BusinessRegistrationForm(instance=(business if edit else None), user=request.user)
...
def edit_business(request, pk):
    """
    Allow the owner to edit their business. Admins can also edit.
    """
    business = get_object_or_404(Business, pk=pk)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_superuser:
                return redirect('accounts:admin/admin_dashboard')
            return redirect('accounts:owner/owner_dashboard')
    else:
        form = BusinessRegistrationForm(instance=business)

    return render(request, 'accounts/register_business.html', {'form': form, 'edit': True})
